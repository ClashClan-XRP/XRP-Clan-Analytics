...
import { GITHUB_FEEDBACK, submitSuggestion } from "@/lib/ops/log";
import { cn } from "@/lib/utils";

export function FeedbackBox({ showReview = true }: { showReview?: boolean }) {
    ...
    <Card>
      <CardContent className="flex flex-col gap-4">
        <div>
          ...
          <h2 className="mt-1 font-display text-3xl leading-none">Submit a suggestion or bug</h2>
          <p className="mt-2 text-sm text-muted-foreground">
            Files a ticket on the clan GitHub so leadership can triage it. Name is optional.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {showReview ? (
            ...
            <a href={GITHUB_FEEDBACK} className="text-sm text-muted-foreground hover:text-foreground" target="_blank" rel="noreferrer">
              Open on GitHub
            </a>
          )}
        </div>
        {sent ? (
          <p className="text-sm text-win">Logged. A GitHub issue draft opened so the ticket lives on the clan repo.</p>
        ) : null}
      </CardContent>
    </Card>
  );
